...
#include "../../include/Time/GreenwichSiderealTime.h"
...
cTLE2PosVel::cTLE2PosVel()
{
	...
	// 计算 1950 年的儒略日
	// 简化实现，使用固定值
	m_dfJD1950 = 2433282.5;

	// FK5 CONSTANT VALUE
    m_dfC1 = 1.72027916940703639e-2;
    m_dfC1P2P = m_dfC1 + 2.0 * M_PI;
...
bool cTLE2PosVel::SetOrbitalElements( stSatelliteIOE stIOE )
{
	...
	// 简单的年积日计算实现
        int nYear = static_cast<int>((m_dfRefJD - 2400000.5) / 365.25) + 1900;
        double dfStartOfYear = 2400000.5 + (nYear - 1900) * 365.25;
        nDoY = static_cast<int>(m_dfRefJD - dfStartOfYear) + 1;
...
//	m_dfRefJD = (double) stSatelliteIOE.nIntJD + (double) stSatelliteIOE.nFractionJD * 1.0e-9;
	...
	m_pdfTLE[ 5 ] = m_dfInc0 = (double) stIOE.pnElement1to6[ 2 ] * 1.0e-5 * M_PI / 180.0;
	m_pdfTLE[ 6 ] = m_dfRAAN0 = (double) stIOE.pnElement1to6[ 3 ] * 1.0e-5 * M_PI / 180.0; 
	m_pdfTLE[ 7 ] = m_dfPerigee0 = (double) stIOE.pnElement1to6[ 4 ] * 1.0e-5 * M_PI / 180.0;
	m_pdfTLE[ 8 ] = m_dfMA0 = (double) stIOE.pnElement1to6[ 5 ] * 1.0e-5 * M_PI / 180.0;

	for( int i = 5; i <= 8; i++ ) m_pdfTLE[ i ] *= 180.0 / M_PI;
...
bool cTLE2PosVel::SetOrbitalElements( double *pdfTLE )
{
	...
	// 简化实现，直接计算儒略日
	// 假设年份在 1950-2050 之间
	m_dfRefJD = 2433282.5 + (year - 1950) * 365.25;
	m_dfRefJD += pdfTLE[ 2 ];

	m_dfBSTAR = pdfTLE[ 3 ];

	m_dfEcc0 = pdfTLE[ 4 ];
	m_dfInc0 = pdfTLE[ 5 ] * M_PI / 180.0;
	m_dfRAAN0 = pdfTLE[ 6 ] * M_PI / 180.0; 
	m_dfPerigee0 = pdfTLE[ 7 ] * M_PI / 180.0;
	m_dfMA0 = pdfTLE[ 8 ] * M_PI / 180.0;
	m_dfMM0 = pdfTLE[ 9 ];
...
//	m_dfSM0 = dfa1 / ( 1.0 - dfDelta0 );
    ...
    m_dfGSTAtRefEpoch = SatelliteOverpass::Time::GreenwichSiderealTime::ComputeGST( m_dfRefJD );
...
FOURTY:
	...
	m_bDeepSpace = false;        // initially, assuming deep space effect ignored
        if( 2.0 * M_PI / m_dfMM0 >= 225.0 )    // satellite higher than 5876 km ( m_bDeepSpace == true )     
	{
		m_bDeepSpace = true;
...
bool cTLE2PosVel::SGP4( double dfJD, double *pdfPos, double *pdfVel )
{
	...
	m_dfRAANM = fmod( m_dfRAANM, 2 * M_PI );
	m_dfPerigeeM = fmod( m_dfPerigeeM, 2 * M_PI );
	dfXLM = fmod( dfXLM, 2 * M_PI );
	m_dfMAM = fmod( dfXLM - m_dfPerigeeM - m_dfRAANM, 2 * M_PI );
    ...
    double dfU = fmod( dfXL - dfRAANP, 2 * M_PI );
	double dfE01 = dfU;
...
bool cTLE2PosVel::InitialiseDeepSpace()
{
	...
	m_dfZMOL = fmod( 4.7199672 + .22997150 * m_dfDSDay - m_dfDSGAM, 2 * M_PI );
	m_dfZMOS = fmod( 6.2565837 + .017201977 * m_dfDSDay, 2 * M_PI );
...
/*************************************************************************************************/
	...
	if( m_nIREZ == 2 ) // GEOPOTENTIAL RESONANCE FOR 12 HOUR ORBITS 
	{
		...
		m_dfXLAMO = fmod( m_dfMA0 + m_dfRAAN0 * 2.0 - m_dfGSTAtRefEpoch * 2.0, 2 * M_PI );
	}
	else	// SYNCHRONOUS RESONANCE TERMS, 24 hour orbits
	{
		...
		m_dfFASX4 =2.8843198;
		m_dfFASX6 =.37448087;                                                     
		m_dfXLAMO = fmod( m_dfMA0 + m_dfRAAN0 + m_dfPerigee0 - m_dfGSTAtRefEpoch, 2 * M_PI );
	}
...
bool cTLE2PosVel::DSCOM()
{
    ...
    double dfXNODCE = fmod( 4.5236020 - 9.2422029e-4 * m_dfDSDay, 2 * M_PI );
...
{
	m_dfDNDT = 0.0;
	double dfTHETA = fmod( m_dfGSTAtRefEpoch + m_dfTC * m_dfEarthRotationPerMinute, 2.0 * M_PI );
...
{
	double dfGST = SatelliteOverpass::Time::GreenwichSiderealTime::ComputeGST( dfJD );
	...
	pdfV[ 0 ] =  pdfVel[ 0 ] * dfCos + pdfVel[ 1 ] * dfSin + pdfP[ 1 ] * m_dfEarthRotation;
	pdfV[ 1 ] = -pdfVel[ 0 ] * dfSin + pdfVel[ 1 ] * dfCos - pdfP[ 0 ] * m_dfEarthRotation;
	pdfV[ 2 ] =  pdfVel[ 2 ];
...
//	if( !CheckSum( line ) ) return false;
	...
	// 简单的日期时间到儒略日转换实现
		// 简化计算，使用固定值
		dfRefJD = 2400000.5 + (nYear - 1900) * 365.25;
